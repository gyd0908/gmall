...
public class SkuInfoController {
    ...
    public Resp<PageVo> queryInfoById(QueryCondition queryCondition,
                                      @RequestParam("catId") Long cid){
        PageVo pageVo=   skuInfoService.queryInfoById(queryCondition,cid);
        return Resp.ok(pageVo);
    }
    @GetMapping("{spuId}")
    public  Resp<List<SkuInfoEntity>> queryBySpuId(@PathVariable("spuId")Long spuId ){

        List<SkuInfoEntity> entities = this.skuInfoService.list(new QueryWrapper<SkuInfoEntity>().eq("spu_id", spuId));
